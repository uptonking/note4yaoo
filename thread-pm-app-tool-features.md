...
- ## 想整理下浏览器书签，才发现原来 Raycast 可以直接搜索书签。
- https://twitter.com/mr_easonyang/status/1647182327964135424
  - 感觉书签整理不是很有必要了，我想甚至可以把他当成轻量级的知识管理工具来用。

- 不得不说 2023 年的今天，书签、剪藏和稍后读三个场景之间的边界越来越模糊了。
- 不得不说 2023 年的今天，书签、剪藏和稍后读三个场景之间的边界越来越模糊了。
  - https://raindrop.io 和 https://pinboard.in 还勉强算是和书签挂点边的产品，而这几年的新产品则几乎没人在意书签的概念了
  - 我本人其实也很早之前就放弃折腾书签直接使用浏览器的同步了
- 对于书签, 我最想要三个功能:
  1. 我最新添加的书签列表
  2. 我最常使用的书签列表
  3. 书签按网站图标形式排列
...
- colab 真是太方便了，不用自己搭环境，GPU 强大，免费分配的都够用

- 按照作者的话说：“根据我们使用若干视频的测试结果，输出字幕的识别准确率可以达到90%以上，这无疑能极大地节省听译及打轴所需的人手和时间。但本应用的目标并非产生完美的字幕文件， 而旨在于搭建并提供一个简单且自动化的使用平台。”
- 英语的话有一个assemblyAI，也是相当厉害，识别准确率应该有99%吧。
  - 看了下价钱，比OpenAI贵一倍，还不像OpenAI能本地离线跑
