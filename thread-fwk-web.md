...
- ## 为什么所有现代 UI 框架都在采用声明式 UI？写起来复杂度低？
- https://x.com/yoshino_2333/status/2048358100324016333
- 图形界面状态转移的关系太多了，从业人员很多也是普通人类，当然还是要声明式描述各种状态，让编译器推导和生成乘法数量的状态之间切换的具体行为。跟你用计算机的时候不想去关系内存怎么申请从哪开始多少字节什么时候释放大差不差。

- 安全和效率。声明式里面的view元素都是immutable，原则上渲染的效率会高而且优化会更好。

- 补充：不只是UI框架，Ops这里基本上也都是声明式的了，像K8s、NixOS、Ansible、Terraform都是声明式的
- 运维领域也是在往声明式发展 声明式可以隐藏复杂度 易于维护吧
