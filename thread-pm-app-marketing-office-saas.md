...
- ## http://elmo.chat 在发布 2 个月后，用户量突破了 3w
- https://x.com/yadong_xie/status/1792750080283603294
- 在经过 22 个版本的迭代后，几乎浏览器能打开内容都可以让 Elmo Chat 先帮你看看来节省时间
  - 精读论文：Arxiv /  PDF
  - 速览新闻：Hackernews / BBC / NYT
  - 总结视频：Youtube / Bilibili
  - 审阅简历：PDF / HTML / Google Doc
  - 不限次数免费 + 不需注册可用 + 不记录网页数据 + 0 后台运行
- 需要自己配置key吧
  - 不用
- 我还发现可以打开小红书的一篇笔记，可以分析评论内容，好用
- 挺好的。要是能总结成脑图就更好了。比Slax Reader总结的好很多。
...
- https://x.com/pingchn/status/1792722509365944450
  - 而国外的这些软件，你有很多选择，你可以选择八十分，你可以选择九十分的。然后它们都可以相互连接起来。我想这个就是差距吧。
- 但是不同产品相互连接的部分可能只有20分…有一说一，办公场景下长期、大量使用的情况，我还是喜欢all in one的产品（也可以特指飞书）。我2022年做了非常详细的调研和报告（也包括slack和teams在内），主导了富途从企业微信切到飞书。实话实说飞书真的很好用，大多数模块功能深度做得很好，而且不同模块（聊天 文档 知识库 日程 会议 邮件）之间的联动体验没有第二个产品做到飞书这样的程度。
  - 飞书体验除了多维表格没有觉得做的多好，只是说隔壁几个对手做的太垃圾了

- 国内的太闭塞了。每家都不够开放。都是在自己的生态里不断限制别人。而不是寻求共赢。所以出现了很多四处缝合的机会，但缝合完了还会面临法律风险。
  - 国内做东西之前首先想壁垒，思维模式决定了喜欢 all in one

- 我觉得可以相互连接起来才是真正的差距（也才是重要的），因为它需要早期就制定标准，大家都follow标准就可以（比如各种rfc），这也是设计哲学范畴问题。 我觉得我们国内软件碰壁几次后，也会向这个方向靠拢。早期时 IE，Firefox，Chrome，Safari也不兼容。现在也好很多。有个过程。

- 开放？那是没被腾讯毒打。Chinaren校友录，人人网被大厂抄袭后弄成死样子。不过wps的应用服务还是有用了其他公司的：比如万兴亿图的思维导图，浩晨的cad，
