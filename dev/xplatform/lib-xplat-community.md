...
- ## 既然代码已经免费，而electron的运行效率又如此之低，为什么客户端的主流开发方式还是不用原生呢。
- https://x.com/huangtao/status/2062668300409295225
- 3个桌面端+2个移动端，不同系统版本还要兼容的话说10x工作量不为过！都用原生的话不管你用不用AI，够你吃一壶。

- 正如不是所有原生上的效果都可以在 web 上做到一样，web 里也有一些东西是原生做不到的
  - web 起码是一个不太会出错的方案，并且能够在不同的平台上保证一个相对一致的体验，这点是所有的原生都做不到的。
  - web 是个虽然有一堆问题，但是久经考验，也是大家见得最多的一个兼容层，兼容层和原生必然有差异。

- 市场决定：先推出一个将就推出（用户也就那样）版，然后看情况迭代，他们打心眼里压根不值得用原生。连操作系统都是这么在更新，何况应用软件，你始终要相信：这个世界就是一个巨大的草台班子，没错。

- electron 兴起因为2 个原因，1是设计稿还原效果不如web，2是oc/c++程序开发慢人也难招
# discuss
- ## 
