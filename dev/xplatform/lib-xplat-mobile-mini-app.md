...
- ## 微信小程序真是一个深坑，不支持svg，不支持 eval，不能使用标准 DOM 事件，甚至连基本CSS特性都有缺失，越写越觉得恶心。
- https://x.com/iamcheyan/status/1984309450316911061
- 腾讯： 你们提供内容和功能就好了，不需要你们提供用户体验

- 表面上和web类似, 似乎能复用web的很多生态和工具. 然后这里机制变一变, 那里来个不支持, 结果就是一锅夹生饭. 除了少部分纯JS的工具, 其它都需要魔改, 而且也不一定能适配(至少3年前是这样的) 也是一点不想再碰; 能想到不多的优点就是提高了政府/官方组织的应用机制/UI的下限

- 永远不搞小程序，最多套个壳接一个webview在里面。以后做任何项目都不优先考虑小程序

- 先前Canvas 支持有问题，文档还没标注，搜了一圈社区才知道咋回事

- 就是要让开发者和开发工具绑定死，离开这这工具到处都是深坑，很多黑箱操作只能在这里面看。网页想自定义分享卡片到微信消息/群聊，当年快被折腾死了

- 我基本不做小程序，标准化程度太低、审核严，非常麻烦

- 不是说底层渲染是魔改的flutter的引擎吗？ 开发不友好，但用户友好呀，点开就用，用户认证、支付，都直接集成，Web上没有一个能替代这东西的。
  - 并不是魔改 flutter，可以参考小程序文档里有实现的表述，是 native view 和  webview
