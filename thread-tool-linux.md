...
- ## 熊出没API技术交流群: 以后找我远程 我不忙的情况下都可以 
- 为什么最近发现远程工具都用uu了
  - 没有会员， UU的不降速
  - 不降速 没有会员 还要啥？都是满血的 不降低画质 我打游戏都用这个以前
- UU没有地域限制
  - 我试过国内远程美国VPS很流畅

- ## [网易UU远程还挺良心的？ - LINUX DO _202606](https://linux.do/t/topic/2399654/8)
- 目前最良心的免费远程软件了吧, 好像已经免费两年了
- UU远程确实好用，目前已经用了一个月高强度远程办公，卡大多数也是公司网差。他家能免费给到原生画质就很不错，希望不要最后良心变质。
- 迟早会收费，UU体验蛮好的。现在上班高频使用不断开10小时+
- 很流畅很稳定, 还能远程开机啥的, 就是感觉画质要是能再高清点就好了, 哪怕付费

- 不得不说，win远程连接mac的键盘逻辑做的比向日葵好太多了

- uu真的太香了，todesk开了会员还有限制
- todesk 一开始好用 后面越改越恶心， 免费给固定时长。 那句话咋说来着 屠龙少年终成恶龙
...
- 以前用teamview，限速了，后来用todesk，免费的体验太差了，再后来自建rusrdesk，没有国内的服务器，速度不行，现在换uu了，体验还好，不知道能坚持多长时间！

- todesk/向日葵连香港或者国外需要单独买插件或者订阅，uu免费

- 一直用这玩意win远程mac studio来着。全屏幕之后，华为win笔记本和mac没差。最近又加了端口转发还有远程终端，相当良心了。内网自动走p2p太强了

- 有触屏模式，用着还挺好，适合远程vibecoding，不用困在主机前

- ## [远程控制手机打卡，推荐装哪个软件？被控端是安卓 - LINUX DO _202605](https://linux.do/t/topic/2202842)
- uu远程 挺好用的 还没有广告。

- 有没有 root，打卡地方有没有电脑？有电脑开 adb + scrcpy 远程电脑打卡
- 没 root 也可以开 adb，然后就可以了

- 连接个pc 然后adb就可以了。。。pc可以通过命令实现远程打卡

- 单纯打卡可以用tailsc+甲壳虫adb。我之前是为了查看短信用的。rustdesk是重点关注软件，好好的一个开源软件，被诈骗分子利用太多了。人家都主动停用了大陆ip
- 电脑用adb即可，手机不需要装软件，前提是你有一个可以联网并且远程控制的电脑。

- 我直接 adb+py 脚本全自动，打完卡把截图发到邮箱，定时检测设备是否在线，范围内随机时间打卡

- 我用rustdesk也被打电话了，线下签字加拍照
  - 有特征流量，走了就会被监控，我也遇到了这个事情
- 之前也因为RustDesk被反诈打电话，问有没有转账啥的，最后去派出所签个字，拍了张照完事
- 因为 RustDesk 被诈骗分子魔改了很多远控软件，它心跳包特征很明显，所以很容易在网络侧触发预警机制。
- rustdesk是开源的，很多诈骗的诱导老人下载进行诈骗，现在rustdesk应该是进入黑名单了，触发了风控的话这个是最高级别预警
  - 麻烦的就是我们正常使用的也会受到影响，本来就是看todesk等三方软件限制太严重，限速也非常狠，搞得自己部署rustdesk用起来也麻烦
- 我都是拒接，一开始是都是机器人给我打电话，听了几次之后都拒接了，也没人继续打了。我是组网ip直连，没用云服务器中转

- 我用iphone的rustdesk，server在腾讯云上，也被96110打电话了，不过是个ai，问几个问题就挂了
  - 倒是之前用三星手机的时候装了个开源聊天工具，96110直接让片区民警上门签反诈的知情书

- 自己用 rustdesk 时候换个 package id 啊，就算反诈不找你，被打卡软件也是精准识别 rustdesk 的，早就在名单里了

- todesk好像也能无root控手机，佬友可以试试

- 被控端安卓装个termux，搞个frp，ios用scrcpy remote，用adb远程调试模式。安卓没root也可以，就是要每周重新开一下端口

- 向日葵下载过，要会员才能远程共享屏幕

- 推荐一个软件：andromeld手机插公司电脑上，远程VNC到公司电脑
# discuss-os-linux-choices
- ## 
...
- ## [linux 系统推荐 - LINUX DO _202606](https://linux.do/t/topic/2416314)
- 最近公司开发主力机器刚装了 deepin , 他的应用商店里面的软件可以满足绝大多数需求，少了很多麻烦，主要做 web android 开发，有点小问题但都还能接受。

- ubuntu商业化好，闭源的显卡驱动支持好，debian崇尚自由软件所以优先支持社区的开源驱动，装闭源的官方驱动比较折腾

- Ubuntu：市场占有率最高，对小白最友好，有问题在各种论坛也基本能找到解决办法；但是我被小bug坑过，现在是ubuntu的黑粉；
arch Linux：配置过，玩过，激情过后索然无味，没用过工作，主要是在安装中学到点知识吧；
opensuse leap / Tumbleweed：我对德国佬的东西有种莫名的信任，现在我的计算服务器还用的 openSUSE leap，稳定好用；有一台老机器用的Tumbleweed，还不错。
fedora：可能是最现代的的发行版？linus诚挚推荐？我在尝试使用他的原子版，配合vibe coding 工作；平时使用kde版本就行，和linus用同款系统。
关于显卡驱动：完全不用担心，都6202年了，驱动都很好打

- CachyOS，我工作都在用。它是基于 Arch Linux 的发行版，常用软件都针对 CPU 指令架构做了编译优化。Arch 系的一个优点就是 aur，直接 yay -S 就可以安装绝大多数软件，不像其他发行版需要自己去下载安装包安装。

- 用过ubuntu，现在个人长期用archlinux，显卡、cuda之类的都能正常使用。
  - ubuntu一个大版本（比如24.04）的官方源软件版本比较稳定，不太适合追新；以前想要更新的官方源需要升级发行版（24.04升级到26.04这种），通常要重装系统，不知道现在有没有更好的方案。
  - archlinux属于滚动更新的发行版（系统由最新软件包组成，没有固定版本号），大多数软件包都能够用上最新的版本；同时社区有AUR（Arch User Repository），提供了大量不在官方仓库的包，用起来很舒服。
  - 缺点是偶尔会出现bug/兼容性问题，需要有一定的问题排查能力和动手能力。

- 感受不出来差别就对了，不同发行版之间没有本质区别。linux系统别当win系统用，linux系统是以cli命令为基础的。你会发现，遇到的问题都能以cli的方式解决，cli用得多，你就察觉到不同发行版之间的细微差别了。

- 我现在是rocky linux 稳定性 感觉还行 但是安装老一些东西的话 各种问题
