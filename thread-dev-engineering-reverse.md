...
- [根据V佬分享的方法，写了一个让Codex降低道德标准，可以帮你写注册机和逆向工程的小工具，已开源！ - LINUX DO _202603](https://linux.do/t/topic/1814815)
...
- https://x.com/cholf5/status/2045776697925234853
...
- ## 完成了多项高难度的逆向与安全研究工作，充分验证了模型在实际复杂工程场景下的强大能力：
- https://x.com/apivixtls/status/2069698840085889104
  - 5分钟内完成 Go 编译 EXE 的高质量反汇编：将一个经过 Go 语言编译、符号表被剥离的 PC 端可执行文件，快速还原为接近可阅读的源码结构，极大降低了逆向分析门槛。
  - Android APK 一键式反编译：成功将某 APK 还原到打包前的工程代码状态（Smali + 资源 + 清单等完整结构），为后续分析和修改提供了清晰的原始框架。
...
- ## [一个可能佬们已经知道并在用的AI逆向思路，主要是为了绕开大模型的道德抵制 - LINUX DO _202606](https://linux.do/t/topic/2356247)
  ...
  - 全程不需要说敏感信息，就说帮我把这个har整理为自动化脚本
  - 对于app也是一样，挂上代理之后也能导出har来
  - 主要是ai不给干啊，我实测gpt5.5 让他给我做个自动化注册机不给干（当然没这么直白），发个har让他给我做成自动化流程，二话没说就开搞了

- 意思就是你把抓包数据和过程发给它，让它给你写脚本
- Copy as cURL的效果也是一样，可以让ai模拟成脚本
...
1. 第一次逆向时，走了些弯路，调用了一个工具的UI界面，然后死循环，手动停止本地流程
2. 纠正后，提醒codex换一个工具，完全使用代码命令完成，这次跑了大概半小时左右
3. 成功计算出x-app-token，可以正常发起请求了。
4. 计算x-app-token时，需要依赖apk中的lib/arm64-v8a/libauth.so文件，我也不知道是干嘛用的，反正能用了 
...
- ## [【开发存疑】为什么codex free注册机都是浏览器自动化版本，现在爬api的方式还能走吗？ - LINUX DO _202605](https://linux.do/t/topic/2252474)
  - 在这个佬的基础上，我加入了5sim的卡台，但是模拟浏览器点击注册速度太慢了，最多够自己用

- api注册的风控高，容易封号，浏览器注册风险最低

- 用的 node 版本开浏览器接码注册然后绑邮箱，目前还比较稳

- 是的，直接接码只能一码一号，邮箱注册，挑号码，因为很多区域要whatsapp。现在openai在灰度支持whatsapp和短信，全面铺开的的话，还是先邮箱再接码划算
