...
- ## [[开源]双图隐写：用两张图片共同隐藏和读取一段文字-隐性水印 - LINUX DO _202606](https://linux.do/t/topic/2451779)
  - 用两张图片共同隐藏和读取一段文字的小工具。
  - 用两张图片共同隐藏一段短文字，读取时也必须两张图片同时在场，才能还原出原文。
  - 注意这是个小玩具，不是专业取证隐写，也不是严肃的加密工具。
  - 短文字 ↓ 编码成 payload ↓ 加入版本、长度、校验信息 ↓ 通过 XOR 拆成两份 share ↓ share A 写入图片 A share B 写入图片 B ↓ 读取时两份重新 XOR 合并 ↓ 还原文字
  - 两张图 + 工具 = 原文
  - 隐写部分用的是图像频域相关思路，把信息写入图片的中频系数区域，并配合纠错、重复映射、校验等机制，尽量让肉眼看不出明显变化，同时提高普通头像压缩、轻微裁剪后的读取成功率。
  - https://github.com/zhuchenyu2008/Dual-image-security-mark
