...
- https://x.com/halfamonk/status/1901554972690612381
  - 然后再到所有的class里面找第二遍，看看谁的内部声明说用了谁，从map里，根据class的名字string，找到类，拿出来，塞需要的class里去？
  - tm离谱（ 这种灵活程度堪比我在python里eval
- 比如C#，不也是服务容器会在启动时会把注册过的依赖加载到内存（复杂版的map），需要的时候根据信息来解析拿去或者实例化，还加上了cache。PHP的laravel也是类似的。
- 其实我对Spring这种pattern没啥意见，唯二不爽的是
  - 1）因为它，spring给我一种“你就负责这个模块吧，整体流程交给我了你别管了”的自负的感觉
  - 2）di导致stack超级深，一个http请求可以有200+层，难看死了
  - asp. net core的stack也这么深吗？

- laravel 借鉴了 Spring，就是PHP版的Spring。
- php 的 laravel 框架也有个类似，就是执行控制台脚本的时候，要执行一个脚本，就要把Command 目录扫个遍，然后才能找到你要执行后脚本，如果在找到要执行的脚本前，有个其它脚本初始化报错，那不好意思，挂了，注意是其它脚本初始化报错，从而影响到要执行的脚本。

- fastapi不是也一样。大差不差啊

- 简单粗暴的解决了问题，虽然略微降低了一点运行时的可靠性，但是工程上完全可以接受

- 三级缓存，嵌套依赖，earlyFactoryBean factoryBean 头疼 名字根本记不住

- spring真是商业外包开发的好朋友，难怪印度的外包开发公司这么喜欢
- 写业务依赖注入很方便还有做测试

- 我觉得 feathersjs 里面的 app.set() app.get() app.configure 也挺好用的。
